...
export const PARTY_IMPORT_CONFIGS: Record<ImportParty, PartyImportConfig> = {
    ...
    // LH3 (the Lead Hunt): leads — a create goes through the lead's own Create (the identifier, the quoted estimate, the source, the score), a merge fills the empties.
    leads: {
        party: "leads",
        singular: "lead",
        plural: "leads",
        section: { label: "Leads", href: "/leads" },
        href: "/leads/import",
        targetHref: (id) => `/leads/${id}`,
        liveDomain: IMPORT_PARTY_DOMAIN.leads,
        formatKind: "leads",
        columns: [
            { key: "businessName", label: "Business", required: true, hint: "The shop, the wall's owner, the brand.", example: "Cloud Nine Cafe" },
            MOBILE,
            { key: "side", label: "Side", required: true, hint: "PUBLISHER (a surface to list) or ADVERTISER (a buyer of attention) — any casing.", example: "PUBLISHER" },
            { key: "category", label: "Category", hint: "Free text; matched against the scoring table (gym, cafe, clinic …).", example: "Cafe" },
            { key: "contactName", label: "Contact", hint: "Free text.", example: "Ravi Kumar" },
            { key: "email", label: "Email", hint: "Lower-cased on the way in.", example: "ravi@example.in" },
            { key: "address", label: "Address", hint: "Free text; geocoded with the city when no coordinates come.", example: "12, 5th Cross, Koramangala" },
            { key: "locality", label: "Locality", hint: "Free text.", example: "Koramangala" },
            { key: "city", label: "City", hint: "Resolved against the catalogue; a city whose lead feeds are off is refused.", example: "Bengaluru" },
            { key: "latitude", label: "Latitude", hint: "Decimal degrees, with the longitude.", example: "12.9352" },
            { key: "longitude", label: "Longitude", hint: "With the latitude.", example: "77.6245" },
            { key: "interest", label: "Interest", hint: "What they said they wanted.", example: "Wall panels" },
            { key: "source", label: "Source", hint: "A label; a new one becomes a source row. Empty is the import.", example: "Trade fair Sept" },
            { key: "bestTimeFrom", label: "Best time from", hint: "HH:MM.", example: "11:00" },
            { key: "bestTimeTo", label: "Best time to", hint: "HH:MM.", example: "14:00" },
            { key: "estimatedCommission", label: "Est. commission", hint: "Rupees; empty quotes the platform's own rate.", example: "1450" },
            { key: "importance", label: "Importance", hint: "STANDARD, KEY or ENTERPRISE — routed to the grade the settings ask for.", example: "STANDARD" },
        ],
        grid: [
        ...
        templateFileName: "leads.csv",
        notePlaceholder: "The trade-fair list, September",
        commitDescription: ({ createdCount, mergedCount }) => perRow(createdCount, mergedCount, "lead", "a LED- number, the quoted estimate, its source row and a score"),
        commitToast: "Each has its LED- number, its score and its source; the nearest agent with room takes the routed ones.",
        rules: {
            title: "Errors are skipped, warnings import with gaps",
            body: "A mobile already on a lead merges into it, filling only what is empty; a mobile on a publisher or advertiser account is invalid — an account is not a prospect; a business name and city already seen is a warning and still creates. Fix the file and upload it again to include the invalid rows.",
        },
        api: () => partyImportService("leads"),
    },
};
