...
/**
 * One agent's KYC, recorded on their behalf and then decided (D4).
 *
 * The top half is the desk's form: what the agent showed, uploaded slot by
 * slot, saved with `PUT /agent-kyc/:agentId` — an upsert, so ops can record
 * what they have and come back for the rest. The bottom half is the decision,
 * the same shape as every other KYC review: verify, or reject with a reason
 * the agent will read.
 */
export function AgentKycRecord({ agent, kyc, live, onChanged }: AgentKycRecordProps) {
    const [form, setForm] = React.useState<AgentKycDocuments>(kyc?.recorded ?? {});
    ...
    const recorded = AGENT_KYC_SLOTS.filter((slot) => Boolean(form[slot.key])).length;
    const name = agent.name ?? agent.mobile;
    ...
    const save = async () => {
        ...
        setSaving(true);
        try {
            await agentKycService.record(agent.id, form);
            toast.success(`${name}'s documents recorded`, {
                description: `${recorded} of ${AGENT_KYC_SLOTS.length} on file · awaiting review`,
            });
            onChanged();
        } catch (cause) {
    ...
    const decide = async (action: "verify" | "reject") => {
        setDeciding(true);
        try {
            await agentKycService.review(agent.id, action === "verify" ? "VERIFIED" : "REJECTED", reason.trim() || undefined);
    ...
    return (
        <div className="space-y-5">
            ...
            {!live && (
                <Card className="rounded-lg border-border p-5 shadow-none">
                    <p className="text-sm text-muted-foreground">
                        Agent KYC is written to the API and has no fixtures. Turn the KYC domain on to record documents.
                    </p>
                </Card>
            )}
            ...
            <div className="grid gap-4 xl:grid-cols-5">
                <Card className="rounded-lg border-border p-5 shadow-none xl:col-span-3">
                    <div className="flex items-start justify-between gap-4">
                        ...
                        <span className="shrink-0 text-xs text-muted-foreground">
                            {recorded} of {AGENT_KYC_SLOTS.length}
                        </span>
                    </div>
                    ...
                    <div className="mt-4 grid gap-4 sm:grid-cols-3">
                        <div className="space-y-1.5">
                            ...
                            <Select
                                ...
                                onValueChange={(value) => set("govIdType", value as AgentKycDocuments["govIdType"])}
                        ...
                        <div className="space-y-1.5">
                            ...
                            <Select
                                ...
                                onValueChange={(value) => set("addressProofType", value as AgentKycDocuments["addressProofType"])}
                                disabled={!live || saving}
                            >
                    ...
                    <div className="mt-4 divide-y border-t">
                        {AGENT_KYC_SLOTS.map((slot) => (
                            <DocumentSlot
                                key={slot.key}
                                label={slot.label}
                                url={form[slot.key as AgentKycSlot]}
                ...
                <div className="space-y-4 xl:col-span-2">
                    <Card className="rounded-lg border-border p-5 shadow-none">
                        ...
                        {kyc ? (
                            <dl className="mt-3 space-y-2 text-sm">
                                ...
                                {kyc.rejectionReason && (
                                    <div className="rounded-md bg-danger-soft px-3 py-2 text-xs text-danger">{kyc.rejectionReason}</div>
                                )}
                            </dl>
                        ) : (
                            <p className="mt-3 text-sm text-muted-foreground">
                                Save the record first; the decision follows what was recorded.
                            </p>
                        )}
                        ...
                        <Textarea
                            ...
                            placeholder="Reason — required for a rejection; the agent reads it back…"
                    ...
                    <Card className="rounded-lg border-border p-5 shadow-none">
                        ...
                        <p className="mt-3 text-sm text-muted-foreground">
                            Agents are onboarded in person and only ever sign in — nothing in the field app lets them
                            submit their own papers. The record remembers who at ADX recorded it, and the bank proof
                            gates the first payout.
                        </p>
                    </Card>
                </div>
            </div>
            ...
            <ConfirmDialog
                ...
                title={confirmAction === "verify" ? "Verify this agent?" : "Reject this agent's KYC?"}
                description={
                    confirmAction === "verify"
                        ? `${name} will be marked verified against the ${recorded} document${recorded === 1 ? "" : "s"} on file.`
                        : `${name} will see your reason in the field app. The desk can record fresh documents afterwards.`
                }
                ...
                onConfirm={() => {
                    if (confirmAction === "reject" && reason.trim().length === 0) {
                        toast.error("Say why before rejecting — the agent reads the reason.");
